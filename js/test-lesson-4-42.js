...
const names = [...books].sort((firstBook, secondBook) => (firstBook.author).localeCompare(secondBook.author)).
    filter(sortedBook => sortedBook.rating > MIN_BOOK_RATING).map(book => book.author);
...
const getNamesSortedByFriendCount = users => {
    ...
    map(userNamed => userNamed.name);
};
// Change code above this line

console.log(getNamesSortedByFriendCount(testUsers46));
//(7) ['Moore Hensley', 'Sharlene Bush', 'Elma Head', 'Sheree Anthony', 'Ross Vazquez', 'Carey Barr', 'Blackburn Dotson']
...
const getSortedFriends = users => {
    return [...users].flatMap(users => users.friends).filter((friend, index, array) => array.indexOf(friend) === index).
    sort((firstFriend, secondFriend) => (firstFriend).localeCompare(secondFriend));
};
// Change code above this line

console.log(getSortedFriends(testUsers47));
...
const getTotalBalanceByGender = (users, gender) => {
    return [...users].filter(usersCopy => usersCopy.gender === gender).reduce((summBalance, user) => {
        return (summBalance +
            user.balance);
    }, 0);
};
...
console.log(getTotalBalanceByGender(testUsers48, "male"));
console.log(getTotalBalanceByGender(testUsers48, "female"));
